...
export default async (req) => {
  ...
  if (!date) return jsonResponse({ error: 'date query param is required' }, 400);

  const upstream = `https://intervals.icu/api/v1/athlete/${encodeURIComponent(athleteId)}/wellness/${encodeURIComponent(date)}`;
...
export const config = {
  path: '/api/intervals/wellness-put',
};
